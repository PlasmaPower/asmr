...
pub mod nano_rpc_maybe_empty {
  ...
  #[allow(dead_code)] // 3 lines of dead code for serialize support if we ever want it ¯\_(ツ)_/¯
...
impl NanoEngine {
  ...
  >(&self, request: &Request) -> anyhow::Result<Response> {
    ...
    let resp = self
      ...
      .text()
      .await?;
    debug!("Nano RPC call {:?} returned {}", request, resp.replace('\n', ""));
  ...
  async fn publish(&self, block: &Block, subtype: &str) -> anyhow::Result<()> {
    ...
    debug_assert_eq!(res.hash, hex::encode(block.get_hash().0));
    Ok(())
  }

  fn compute_work(root: [u8; 32], threshold: u64) -> u64 {
    info!("Generating Nano proof of work for root {} with threshold 0x{:16x}", hex::encode(&root), threshold);
    ...
    debug!("Generated Nano proof of work nonce {:16x} for root {}", nonce, hex::encode(&root));
    nonce
  }
  ...
  pub async fn send_from_node(&self, destination: &str, amount: u128) -> anyhow::Result<BlockHash> {
    ...
    let request = SendRequest {
      ...
      source: &self.config.wallet_address,
      destination,
